...
class ArtistAddTestTest < ActionDispatch::IntegrationTest
	...
	test "unlogged user adds new artist" do
		...
		assert_no_difference 'Artist.count', 1 do
			post artists_path,  artist: { name: "Name", image: "1.jpg", description: "desc", origin: "WA", genre: "Rock" }
		end
	...
	test "logged user adds new artist with lacking data and then corrects" do
		...
		assert_no_difference 'Artist.count', 1 do
			post artists_path,  artist: { name: "", image: "1.jpg", description: "desc", origin: "WA", genre: "Rock" }
		end
		assert_template 'artists/new'
		assert_difference 'Artist.count', 1 do
			post new_artist_path,  artist: { name: "Name", image: "1.jpg", description: "desc", origin: "WA", genre: "Rock" }
		end
		assert_template 'artists/show'
	end
